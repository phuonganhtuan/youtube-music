...
import com.example.videomusic.databinding.ActivityMainBinding
import com.example.videomusic.ui.createpl.CreatePLFragment
import com.example.videomusic.ui.playlist.RecentFragment
import com.example.videomusic.utils.NetworkUtil
        ...
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            mediaService?.exoPlayer?.addListener(object : Player.EventListener {
                ...
                override fun onTracksChanged(
                    ...
                    if (mediaService!!.recentVideo == null) return
...
//            .into(imageBg)
    ...
    private fun setAudioDataRecent(video: String, list: List<String>) {
        currentList = list
        mediaService?.recentVideo = video
        mediaService?.prepareDataFromLocal(list)
        isPlaying = true
        displayPlayingState()
    }

    private fun setupNewVideoViews(video: String) {
        viewBinding.apply {
            textTitle.text = video
            val thumbnail = Uri.fromFile(File(video))
            Glide.with(this@MainActivity)
                .load(thumbnail)
                .into(imageAvatar)
            Glide.with(this@MainActivity)
                ...
                .apply(bitmapTransform(BlurTransformation(100, 3)))
                .into(imageBg)
            val duration = File(video).getMediaDuration(this@MainActivity)
            textLyrics.text = video
            seekBar.show()
            textProgress.show()
            textDuration.show()
    ...
    private fun File.getMediaDurationString(context: Context): String {
        if (!exists()) return "---"
        val retriever = MediaMetadataRetriever()
        retriever.setDataSource(context, Uri.parse(absolutePath))
        val duration = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)
        retriever.release()
        val longDuration = duration?.toLongOrNull() ?: 0
        return String.format(
            "%02d:%02d:%02d", TimeUnit.MILLISECONDS.toHours(longDuration),
            TimeUnit.MILLISECONDS.toMinutes(longDuration) % TimeUnit.HOURS.toMinutes(1),
            TimeUnit.MILLISECONDS.toSeconds(longDuration) % TimeUnit.MINUTES.toSeconds(1)
        )
    }
    ...
    private fun handleEvents() = with(viewBinding) {
        ...
        imagePlayAndPause.setOnClickListener {
            if (mediaService?.recentVideo == null) return@setOnClickListener
            isPlaying = !isPlaying
            if (isPlaying) {
                displayPlayingState()
                mediaService?.let {
    ...
    private fun openRecent(item: String, list: List<String>) {
        viewBinding.progressBar.show()
        viewBinding.progressBar.show()
